...
namespace sio
{
    ...
    unsigned client_impl::next_delay() const
    {
        ...
        return static_cast<unsigned>(min<double>(m_reconn_delay * pow(1.52,reconn_made),m_reconn_delay_max));
    }
    ...
    void client_impl::on_fail(connection_hdl con)
    {
		...
		if(m_reconn_made % 4 == 0)
		{
			...
			return ;
			//m_base_url = m_temp_url_a;
			//m_temp_url_a = m_temp_url_b;
			//m_temp_url_b = m_base_url;
		}
...
failed:
	...
	void client_impl::sethost(char* _pHost)
	{
		pHost = _pHost;
	}
